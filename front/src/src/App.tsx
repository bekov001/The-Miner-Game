...
function App() {
  ...
  const earn = (block: string, pos: number)=> {
    ...
    if (block === 'community' || friends.length >= 3 || block === "chat"){
  ...
  const upgrade = (block: string) => {
    ...
    const mine = () => {
        ...
        if (!winDrop){
          ...
          // setRockH
          if (pickaxeP[1] === 1 && (clicks + 1) % 2 === 0 && rockNumber <= 17){
            setRockNumber(rockNumber + 1)
          } else if (pickaxeP[1] === 2 && rockNumber + 1 <= 17){
            setRockNumber(rockNumber + 1)
          } else if (pickaxeP[1] >= 3 && rockNumber + pickaxeP[1] - 1 <= 17){
            setRockNumber(rockNumber + pickaxeP[1] - 1)
          } else if (rockNumber + pickaxeP[1] - 1 > 17) {
